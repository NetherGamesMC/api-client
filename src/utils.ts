...
export function parseLeaderboardColumn(
  ...
  if (game == null && type == null && isSubtypeValid) {
    return {success: true, column};
  }

  const gameRef = LEADERBOARD_COLUMNS_NESTED[game as keyof typeof LEADERBOARD_COLUMNS_NESTED];
  if (gameRef == null) {
    return {success: false, values: getObjectKeys(LEADERBOARD_COLUMNS_NESTED)};
  }
  ...
  if (typeRef == null) {
    ...
    if (subtype != null && isSubtypeValid) {
      for (const subtype of POSSIBLE_SUBTYPES) {
        delete gameKeys[subtype];
      }
    }
  ...
  if (subtypeRef == null) {
    return {success: false, values: getObjectKeys(gameRef)};
  }
